...
def generate_single(model, processor, image_path: str, question: str, temperature: float = 0.7, max_new_tokens: int = 256, use_unsloth: bool = False, model_type: str = "qwen"):
    """Generate one answer. Supports Qwen2.5-VL and MedGemma."""
...
def generate_answers_transformers(
    ...
    cfg = MODEL_CONFIGS.get(model_name, (model_name, "qwen"))
...
def generate_answers_with_layer_dynamics(
    ...
    if model_type != "qwen":
        raise NotImplementedError(
            "Layer dynamics supports qwen2.5-vl-7b only"
        )
    model_id = cfg[0] if isinstance(cfg, tuple) else cfg
